...
class RelativeDistanceQAGeneratorV3(BaseQAGenerator):
    """
    Generates frame-level multiple-choice questions (2 options) asking which object,
    ...
    This is the V3 version with 2 choices.
    """
    def __init__(self):
        super().__init__()
    ...
    def get_default_args(self):
        """Return default arguments specific to frame-level relative distance QA (V3 - 2 choices)."""
        return {
    ...
    def _format_qa_item(self, scene_name, scene_info, frame_id, frame_rank, num_frames, potential_options):
        ...
        for cand in options_candidates:
            is_ambiguous = False
            # For 2 choices, ambiguity is only checked against the GT or other selected distractors.
            # The loop correctly handles this: if num_distractors_needed is 1, it checks against GT.
            # If more, it checks against GT and already selected distractors.
    ...
    def generate_scene_qa(self, scene_name, scene_info, frame_info_for_scene):
        ...
        # num_valid_frames = len(valid_frames) # For QA logic if needed, but iteration is on valid_frames
        ...
        for frame_rank, frame_data in enumerate(valid_frames): # Iterate over valid_frames
            ...
            if cam_pos_world is None:
                # This should ideally not happen if pre-filtering worked, but as a safeguard:
        ...
        logger.info(f"Scene {scene_name}: Successfully generated {len(scene_qa_list)} relative distance QA items (V3).")
