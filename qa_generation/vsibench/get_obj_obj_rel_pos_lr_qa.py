...
class ObjectObjectRelativePositionLRQAGenerator(BaseQAGenerator):
    """
    Generates frame-level questions asking for the spatial relationship
    (Left/Right) between two objects that are unique
    by category within that specific frame.
    Camera coordinates assumed: +X Right, +Y Down, +Z Forward.
    """
    def __init__(self):
        super().__init__()
        self.options_lr = ['Left', 'Right'] # Left, Right
        self.option_letters = ['A', 'B']
        self.answer_counts = {'lr': {'A': 0, 'B': 0}} # Simplified for LR only
        self.position_threshold = 0.15 # meters

    def get_default_args(self):
        """Return default arguments specific to object-object Left/Right relative position QA."""
        return {
            'question_template': 'VSTI_OBJ_OBJ_REL_POS_LR_TEMPLATE', # Left/Right template
            'num_subsample': 9, 
            'question_type_prefix': 'obj_obj_relative_pos_lr', # Specific to LR
            'output_filename_prefix': 'qa_obj_obj_relative_pos_lr', # Specific to LR
        }
    ...
    def generate_scene_qa(self, scene_name, scene_info, frame_info_for_scene):
        """Generates frame-level object-object Left/Right relative position QA pairs."""
        scene_qa_list = []
        ...
        # num_valid_frames = len(valid_frames) # For QA logic iteration
        ...
        for frame_index, frame_data in enumerate(valid_frames): # Iterate over valid_frames
            ...
            camera_pose_list = frame_data.get('camera_pose_camera_to_world') # Already checked by filter
            bboxes_2d = frame_data.get('bboxes_2d', []) # Already checked by filter
            ...
            if len(frame_specific_unique_instance_ids) >= 2:
                ...
                for inst_id_A_str, inst_id_B_str in itertools.combinations(frame_specific_unique_instance_ids, 2):
                    ...
                    min_xA, max_xA = min_A[0], max_A[0]
                    min_xB, max_xB = min_B[0], max_B[0]

                    # Left/Right (X-axis: +X is Right)
                    # A is entirely Left of B if max_xA < min_xB (with margin)
                    if (min_xB - max_xA) > self.position_threshold:
                        gt_relation_lr = self.options_lr[0] # Left
                        potential_qas.append({
                            "frame_id": frame_id, "frame_rank": frame_index,
                            "obj_A_cat": cat_A, "obj_B_cat": cat_B,
                            "dimension": "lr", "options": self.options_lr,
                            "ground_truth": gt_relation_lr
                        })
                    # A is entirely Right of B if min_xA > max_xB (with margin)
                    elif (min_xA - max_xB) > self.position_threshold:
                        gt_relation_lr = self.options_lr[1] # Right
                        potential_qas.append({
                            "frame_id": frame_id, "frame_rank": frame_index,
                            "obj_A_cat": cat_A, "obj_B_cat": cat_B,
                            "dimension": "lr", "options": self.options_lr,
        ...
        for qa_info in potential_qas: 
            ...
            shuffled_options = self.options_lr[:]
            random.shuffle(shuffled_options)
            if gt_relation not in shuffled_options:
                 logger.warning(f"Scene {scene_name}, Frame {frame_id}, Pair ({obj_A_cat}, {obj_B_cat}), Dim lr: GT '{gt_relation}' not in options. Skipping.")
                 continue
            gt_index = shuffled_options.index(gt_relation)
            mc_answer = self.option_letters[gt_index]
            self.answer_counts["lr"][mc_answer] += 1
        ...
        logger.info(f"Scene {scene_name}: Generated {len(scene_qa_list)} obj-obj Left/Right QA pairs.")
...
if __name__ == '__main__':
    ...
    generator = ObjectObjectRelativePositionLRQAGenerator() # Use new class name
    # Ensure the specific LR template is available or add a dummy one
    if not hasattr(prompt_templates, 'VSTI_OBJ_OBJ_REL_POS_LR_TEMPLATE'):
        prompt_templates.VSTI_OBJ_OBJ_REL_POS_LR_TEMPLATE = "In {frame_description}, relative to {obj_B_name}, is {obj_A_name} to the [Left/Right]?"
    # Remove NF and UD template checks
    generator.run() 