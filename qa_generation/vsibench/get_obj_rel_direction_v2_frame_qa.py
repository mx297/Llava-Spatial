...
class ObjRelDirV2QAGenerator(BaseQAGenerator):
    ...
    def get_default_args(self):
        return {
            'question_template': "OBJ_REL_DIRECTION_V2_FRAME_TEMPLATE",  # changed
            'num_subsample': 6,
            'question_type': 'object_rel_direction_v2_frame',             # changed
            'output_filename_prefix': 'qa_obj_rel_direction_v2_frame'     # changed
        }

    def generate_scene_qa(self, scene_name, scene_info, frame_info_for_scene):
        """
        Generate frame-wise relative direction V2 QA pairs.
        For each frame, identify categories that are visible exactly once.
        Then form triples (positioning, orienting, querying) to compute the
        relative direction question (left, right, back).
        """
        scene_qa_list = []
        ...
        # Also build category -> {iid -> obj_dict} for metadata lookup
        cat_iid_to_obj = {}
        for category, instances in obj_bboxes.items():
            if not isinstance(instances, list):
                continue
        ...
        for rank, f in enumerate(frames_sorted):
            ...
            # Only categories with exactly one visible instance
            unique_cats = [c for c, s in cat_to_iids.items() if len(s) == 1]
            if len(unique_cats) < 3:
                continue

            # Subsample object triples for performance
            ...
            for object_triple in object_triples:
                ...
                for positioning_obj, orienting_obj, querying_obj in object_triple_permutations:
                    ...
                    qa = {
                        ...
                        "frame_indices": [frame_id],  # frame-wise tracking
