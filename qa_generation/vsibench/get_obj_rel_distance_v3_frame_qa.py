...
class ObjRelDistV3QAGenerator(BaseQAGenerator):
    ...
    def get_default_args(self):
        return {
            'question_template': "OBJ_REL_DISTANCE_V3_FRAME_TEMPLATE",   # changed
            'num_subsample': 6,
            'question_type': 'object_rel_distance_v3_frame',              # changed
            'output_filename_prefix': 'qa_obj_rel_distance_v3_frame'      # changed
        }


    def generate_scene_qa(self, scene_name, scene_info, frame_info_for_scene):
        """
        Generate frame-wise relative distance V3 QA pairs.

        For each frame, consider all visible categories (from bboxes_2d) that appear exactly once.
        For each single-visible category, pick 2 others and ask which one is closest.
        """
        scene_qa_list = []
        ...
        for rank, f in enumerate(frames_sorted):
            ...
            # Keep only categories visible exactly once
            visible_cats = list(cat_to_iids.keys())
            single_visible_cats = [c for c, s in cat_to_iids.items() if len(s) == 1]

            # Need at least 1 single-count and 2 others
            if len(visible_cats) < 3 or not single_visible_cats:
                continue
            ...
            for primary_cat in single_visible_cats:
                ...
                combos = list(itertools.combinations(other_cats, 2))
                for combo in combos:
