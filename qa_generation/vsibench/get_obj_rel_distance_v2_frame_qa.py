...
def get_gt(choice_categories, primary_object_category, obj_bbox_info, 
    ...
    Arg(s):
        ...
        option_letters: list of option letters (e.g., ['A', 'B'])
...
class ObjRelDistV2QAGenerator(BaseQAGenerator):
    def __init__(self):
        super().__init__()
        self.answer_counts = {'A': 0, 'B': 0, 'C': 0}  # 3 options
        self.option_letters = ['A', 'B', 'C']

    def get_default_args(self):
        return {
            'question_template': "OBJ_REL_DISTANCE_V2_FRAME_TEMPLATE",   # changed
            'num_subsample': 6,
            'question_type': 'object_rel_distance_v2_frame',              # changed
            'output_filename_prefix': 'qa_obj_rel_distance_v2_frame'      # changed
        }


    def generate_scene_qa(self, scene_name, scene_info, frame_info_for_scene):
        """
        Generate frame-wise relative distance V2 QA pairs.

        For each frame, consider all visible categories (from bboxes_2d) that appear exactly once.
        For each single-visible category, pick 3 others and ask which one is closest.
        """
        scene_qa_list = []
        ...
        # Build instance_id -> category mapping from metadata
        iid_to_category = {}
        ...
        for rank, f in enumerate(frames_sorted):
            ...
            # Count visible instances per category
            cat_to_iids = {}
            for box in b2d:
            ...
            # Keep categories with exactly one visible instance
            visible_cats = list(cat_to_iids.keys())
            single_visible_cats = [c for c, s in cat_to_iids.items() if len(s) == 1]

            # Need at least one single-count + 3 others
            if len(visible_cats) < 4 or not single_visible_cats:
                continue
            ...
            for primary_cat in single_visible_cats:
                ...
                combos = list(itertools.combinations(other_cats, 3))
                for combo in combos:
            ...
            # Compute distances and format QA items
...
if __name__ == '__main__':
    generator = ObjRelDistV2QAGenerator()
    generator.run() 