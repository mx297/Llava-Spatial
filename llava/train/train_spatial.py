...
class ModelArguments:
    ...
    vision_tower_pretrained: Optional[str] = field(default="")

    # NEW: Spatial / Fusion
    
    spatial_tower: Optional[str] = field(default=None)
    spatial_tower_select_feature: Optional[str] = field(
        default=None, metadata={"help": 'Which spatial features to fuse: "patch_tokens", "camera_tokens", "all"'}
    )
...
def find_all_linear_names(model):
    ...
    # NEW: exclude spatial/fusion modules from LoRA
    multimodal_keywords = ['mm_projector', 'vision_tower', 'vision_resampler',
                           'spatial_tower', 'fusion_block']
...
def train(attn_implementation=None):
    ...
    if training_args.bits in [4, 8]:
        ...
        bnb_model_from_pretrained_args.update(dict(
            ...
            quantization_config=BitsAndBytesConfig(
                load_in_4bit=training_args.bits == 4,
                load_in_8bit=training_args.bits == 8,
                # NEW: keep these modules in full precision
                llm_int8_skip_modules=["mm_projector", "fusion_block", "spatial_tower"],
    ...
    # ---- Initialize vision + (optionally) spatial/fusion modules ----
    if model_args.vision_tower is not None:
        ...
        # LoRA and projector freezing logic (unchanged)
        ...
        # NEW: carry spatial feature selection setting into config
        setattr(model.config, "spatial_tower_select_feature", model_args.spatial_tower_select_feature)

        # NEW: initialize optional spatial tower
        if hasattr(model.get_model(), "initialize_spatial_tower") and (model_args.spatial_tower is not None):
            model.get_model().initialize_spatial_tower(model_args, fsdp=training_args.fsdp)
        ...
        # NEW: initialize optional fusion block
        if hasattr(model.get_model(), "initialize_fusion_block") and (model_args.fusion_block is not None):
            model.get_model().initialize_fusion_block(model_args, fsdp=training_args.fsdp)
    ...
    # ---- dtype fixes when using LoRA/k-bit ----
    if training_args.bits in [4, 8]:
