...
class ObjRelDirV3QAGenerator(BaseQAGenerator):
    def __init__(self):
        # Override init for 2 options
        super().__init__()
        self.answer_counts = {'A': 0, 'B': 0}
        self.option_letters = ['A', 'B']

    def get_default_args(self):
        return {
            'question_template': "OBJ_REL_DIRECTION_V3_FRAME_TEMPLATE",   # changed
            'num_subsample': 6,
            'question_type': 'object_rel_direction_v3_frame',              # changed
            'output_filename_prefix': 'qa_obj_rel_direction_v3_frame'      # changed
        }


    def generate_scene_qa(self, scene_name, scene_info, frame_info_for_scene):
        """
        Generate frame-wise relative direction V3 QA pairs.
        For each frame, find categories visible exactly once (from bboxes_2d),
        then form triples (positioning, orienting, querying) and compute
        left/right relationships using get_gt().
        """
        scene_qa_list = []
        ...
        # Build category -> {iid -> obj_dict} for metadata lookup
        cat_iid_to_obj = {}
        for category, instances in obj_bboxes.items():
            if not isinstance(instances, list):
                continue
        ...
        for rank, f in enumerate(frames_sorted):
            ...
            for object_triple in object_triples:
                ...
                for positioning_obj, orienting_obj, querying_obj in permutations:
                    ...
                    qa = {
                        ...
                        "frame_indices": [frame_id],  # frame-aware
...
if __name__ == '__main__':
    generator = ObjRelDirV3QAGenerator()
    generator.run()